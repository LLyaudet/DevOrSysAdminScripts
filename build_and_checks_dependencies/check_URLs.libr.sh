  ...
  for LFBFL_pattern in "${COMMON_TEXT_FILES_GLOB_PATTERNS[@]}"; do
    ...
    find . -type f -name "${LFBFL_pattern}" -printf '%P\0'\
      ...
      | grep --invert-match --perl-regexp\
        "(['\"])http(:[^'\"]*)\\1.*(['\"])https\\2\\3"\
      | grep --invert-match\
        --regexp='http://www.w3.org/1999/xhtml'\
        --regexp="http://www.w3.org/1999/XSL/Transform"\
    ...
    for LFBFL_file_path in "${LFBFL_arr_files_paths[@]}"; do
      ...
      for LFBFL_substitution in "${!LFBFL_substitutions[@]}"; do
        ...
        if grep --quiet "${LFBFL_substitution}" -- "${LFBFL_file_path}";
        then
          sed --in-place\
          --expression="s|${LFBFL_substitution}|${LFBFL_substitution2}|g"\
            -- "${LFBFL_file_path}"
        fi
      done
    done
