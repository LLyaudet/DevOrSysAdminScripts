...
commit_a_file_renamed_comment(){
  ...
  for ((LFBFL_i=0; LFBFL_i<${#LFBFL_renaming_lines_array[@]};)); do
    ...
    if [[ "${LFBFL_similarity_line:17:4}" == "100%" ]]; then
      ...
      readonly LFBFL_useful_file_name
      ...
      LFBFL_new_comment+=$(date --iso-8601="minute")
      ...
      declare -ir LFBFL_copyright_line_number=$(
        grep -n '©Copyright' "${LFBFL_new_file_name}" | cut -d ':' -f 1
      )
      # shellcheck disable=SC2250
      if [[ $LFBFL_verbose -eq 1 ]]; then
        echo "LFBFL_copyright_line_number: ${LFBFL_copyright_line_number}"
      fi
      if [[ ${LFBFL_copyright_line_number} -eq 0 ]]; then
        declare -r\
          LFBFL_send_summary_2="ATTENTION: No line with copyright"
      ...
      # Update file. -------------------------------------------------
      declare -r\
        LFBFL_temp_file_name="${LFBFL_new_file_name}.LFBFL.temp"
  ...
  if [[ LFBFL_renaming_happened -eq 1 ]]; then
    # Commit ---------------------------------------------------------
    git commit\
      --message="commit_a_file_renamed_comment() post-commit hook."
  fi
}
